...
class ImmiScaryVideoScreen extends StatefulWidget {
  const ImmiScaryVideoScreen({Key? key}) : super(key: key);

  @override
  State<ImmiScaryVideoScreen> createState() => _ImmiScaryVideoScreenState();
}

class _ImmiScaryVideoScreenState extends State<ImmiScaryVideoScreen> {
  @override
...
class ImmiScaryVideo extends StatefulWidget {
  const ImmiScaryVideo({Key? key}) : super(key: key);

  @override
  _ImmiScaryVideoState createState() => _ImmiScaryVideoState();
}

class _ImmiScaryVideoState extends State<ImmiScaryVideo> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _tabController = TabController(length: immiscary.length, vsync: this);
    Wakelock.enable();
  }
  ...
  Widget getBody() {
    ...
    return PageView.builder(
        itemCount: immiscary.length,
        itemBuilder: (context, index) {
          return VideoPlayerItem(
            videoUrl: immiscary[index]['videoUrl'],
            size: size,
            name: immiscary[index]['name'],
            caption: immiscary[index]['caption'],
            profileImg: immiscary[index]['profileImg'],
          );
        });
  }
}
...
List immiscary = [
  {
    "videoUrl": "http://13.229.103.9:3000/videos/video_1660254113820.mp4",
    "name": "Samin Rahman",
    "caption": "",
