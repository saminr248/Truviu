...
final Uri _url = Uri.parse('https://www.deserve.com/apply-for-deserve/?utm_medium=cust_ref&utm_source=invite&utm_campaign=0740F&ref=0740F');
...
class ApplyCCVideoScreen extends StatefulWidget {
  const ApplyCCVideoScreen({Key? key}) : super(key: key);

  @override
  State<ApplyCCVideoScreen> createState() => _ApplyCCVideoScreenState();
}

class _ApplyCCVideoScreenState extends State<ApplyCCVideoScreen> {
  @override
...
class ApplyCCVideo extends StatefulWidget {
  const ApplyCCVideo({Key? key}) : super(key: key);

  @override
  _ApplyCCVideoState createState() => _ApplyCCVideoState();
}

class _ApplyCCVideoState extends State<ApplyCCVideo> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _tabController = TabController(length: applycc.length, vsync: this);
    Wakelock.enable();
  }
  ...
  Widget getBody() {
    ...
    return PageView.builder(
        itemCount: applycc.length,
        itemBuilder: (context, index) {
          return VideoPlayerItem(
            videoUrl: applycc[index]['videoUrl'],
            size: size,
            name: applycc[index]['name'],
            caption: applycc[index]['caption'],
            link: applycc[index]['link'],
            profileImg: applycc[index]['profileImg'],
          );
        });
  }
}
...
class VideoPlayerItem extends StatefulWidget {
  ...
  VideoPlayerItem(
      {Key? key,
        ...
        required this.videoUrl, required this.link})
      : super(key: key);
...
List applycc = [
  {
    "videoUrl": "http://13.229.103.9:3000/videos/video_1660254975366.mp4",
    "name": "Samin Rahman",
    "caption": "Get \$30 by applying with this link: ",
    "link": "https://deserve.com/0740F",
    "profileImg": "http://13.213.0.96/images/image_1657916391818.jpg",
  },
];


class LeftPanelWithCaption extends StatelessWidget {
  final String name;
  final String caption;
  final String link;
  const LeftPanelWithCaption({
    Key? key,
    required this.size,
    required this.name,
    required this.caption, required this.link,
  }) : super(key: key);
          ...
          Row(
            children: [
              ...
              GestureDetector(
                ...
                child: Text(
                  link,
                  style: TextStyle(color: white, decoration: TextDecoration.underline, fontSize: 10),
                ),
              )
            ],
          ),
