...
class WhatCCVideoScreen extends StatefulWidget {
  const WhatCCVideoScreen({Key? key}) : super(key: key);

  @override
  State<WhatCCVideoScreen> createState() => _WhatCCVideoScreenState();
}

class _WhatCCVideoScreenState extends State<WhatCCVideoScreen> {
  @override
...
class WhatCCVideo extends StatefulWidget {
  const WhatCCVideo({Key? key}) : super(key: key);

  @override
  _WhatCCVideoState createState() => _WhatCCVideoState();
}

class _WhatCCVideoState extends State<WhatCCVideo> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _tabController = TabController(length: whatcc.length, vsync: this);
    Wakelock.enable();
  }
  ...
  Widget getBody() {
    ...
    return PageView.builder(
        itemCount: whatcc.length,
        itemBuilder: (context, index) {
          return VideoPlayerItem(
            videoUrl: whatcc[index]['videoUrl'],
            size: size,
            name: whatcc[index]['name'],
            caption: whatcc[index]['caption'],
            profileImg: whatcc[index]['profileImg'],
          );
        });
  }
}
...
List whatcc = [
  {
    "videoUrl": "http://13.229.103.9:3000/videos/video_1660252953259.mp4",
    "name": "Samin Rahman",
    "caption": "",
