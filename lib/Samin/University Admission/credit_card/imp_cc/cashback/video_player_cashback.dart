...
class CashbackCCVideoScreen extends StatefulWidget {
  const CashbackCCVideoScreen({Key? key}) : super(key: key);

  @override
  State<CashbackCCVideoScreen> createState() => _CashbackCCVideoScreenState();
}

class _CashbackCCVideoScreenState extends State<CashbackCCVideoScreen> {
  @override
...
class CashbackCCVideo extends StatefulWidget {
  const CashbackCCVideo({Key? key}) : super(key: key);

  @override
  _CashbackCCVideoState createState() => _CashbackCCVideoState();
}

class _CashbackCCVideoState extends State<CashbackCCVideo> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _tabController = TabController(length: cashbackcc.length, vsync: this);
    Wakelock.enable();
  }
  ...
  Widget getBody() {
    ...
    return PageView.builder(
        itemCount: cashbackcc.length,
        itemBuilder: (context, index) {
          return VideoPlayerItem(
            videoUrl: cashbackcc[index]['videoUrl'],
            size: size,
            name: cashbackcc[index]['name'],
            caption: cashbackcc[index]['caption'],
            profileImg: cashbackcc[index]['profileImg'],
          );
        });
  }
}
...
List cashbackcc = [
  {
    "videoUrl": "http://13.229.103.9:3000/videos/video_1660246552089.mp4",
    "name": "Samin Rahman",
    "caption": "",
