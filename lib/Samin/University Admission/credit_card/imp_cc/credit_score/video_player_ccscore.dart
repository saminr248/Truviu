...
class CCScoreVideoScreen extends StatefulWidget {
  const CCScoreVideoScreen({Key? key}) : super(key: key);

  @override
  State<CCScoreVideoScreen> createState() => _CCScoreVideoScreenState();
}

class _CCScoreVideoScreenState extends State<CCScoreVideoScreen> {
  @override
...
class CCScoreVideo extends StatefulWidget {
  const CCScoreVideo({Key? key}) : super(key: key);

  @override
  _CCScoreVideoState createState() => _CCScoreVideoState();
}

class _CCScoreVideoState extends State<CCScoreVideo> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _tabController = TabController(length: ccscore.length, vsync: this);
    Wakelock.enable();
  }
  ...
  Widget getBody() {
    ...
    return PageView.builder(
        itemCount: ccscore.length,
        itemBuilder: (context, index) {
          return VideoPlayerItem(
            videoUrl: ccscore[index]['videoUrl'],
            size: size,
            name: ccscore[index]['name'],
            caption: ccscore[index]['caption'],
            profileImg: ccscore[index]['profileImg'],
          );
        });
  }
}
...
List ccscore = [
  {
    "videoUrl": "http://13.229.103.9:3000/videos/video_1660246905146.mp4",
    "name": "Samin Rahman",
    "caption": "",
