...
class MattressVideoScreen extends StatefulWidget {
  const MattressVideoScreen({Key? key}) : super(key: key);

  @override
  State<MattressVideoScreen> createState() => _MattressVideoScreenState();
}

class _MattressVideoScreenState extends State<MattressVideoScreen> {
  @override
...
class MattressVideo extends StatefulWidget {
  const MattressVideo({Key? key}) : super(key: key);

  @override
  _MattressVideoState createState() => _MattressVideoState();
}

class _MattressVideoState extends State<MattressVideo> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _tabController = TabController(length: mattress.length, vsync: this);
    Wakelock.enable();
  }
  ...
  Widget getBody() {
    ...
    return PageView.builder(
        itemCount: mattress.length,
        itemBuilder: (context, index) {
          return VideoPlayerItem(
            videoUrl: mattress[index]['videoUrl'],
            size: size,
            name: mattress[index]['name'],
            caption: mattress[index]['caption'],
            profileImg: mattress[index]['profileImg'],
          );
        });
  }
}
...
List mattress = [
  {
    "videoUrl": "http://13.229.103.9:3000/videos/video_1660245163752.mp4",
    "name": "Samin Rahman",
    "caption": "",
