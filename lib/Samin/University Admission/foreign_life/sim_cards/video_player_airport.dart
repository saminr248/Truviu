...
class AirportSimVideoScreen extends StatefulWidget {
  const AirportSimVideoScreen({Key? key}) : super(key: key);

  @override
  State<AirportSimVideoScreen> createState() => _AirportSimVideoScreenState();
}

class _AirportSimVideoScreenState extends State<AirportSimVideoScreen> {
  @override
...
class AirportSimVideo extends StatefulWidget {
  const AirportSimVideo({Key? key}) : super(key: key);

  @override
  _AirportSimVideoState createState() => _AirportSimVideoState();
}

class _AirportSimVideoState extends State<AirportSimVideo> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _tabController = TabController(length: airportsim.length, vsync: this);
    Wakelock.enable();
  }
  ...
  Widget getBody() {
    ...
    return PageView.builder(
        itemCount: airportsim.length,
        itemBuilder: (context, index) {
          return VideoPlayerItem(
            videoUrl: airportsim[index]['videoUrl'],
            size: size,
            name: airportsim[index]['name'],
            caption: airportsim[index]['caption'],
            profileImg: airportsim[index]['profileImg'],
          );
        });
  }
}
...
List airportsim = [
  {
    "videoUrl": "http://13.229.103.9:3000/videos/video_1660246203313.mp4",
    "name": "Samin Rahman",
    "caption": "",
