...
class PlanSimVideoScreen extends StatefulWidget {
  const PlanSimVideoScreen({Key? key}) : super(key: key);

  @override
  State<PlanSimVideoScreen> createState() => _PlanSimVideoScreenState();
}

class _PlanSimVideoScreenState extends State<PlanSimVideoScreen> {
  @override
...
class PlanSimVideo extends StatefulWidget {
  const PlanSimVideo({Key? key}) : super(key: key);

  @override
  _PlanSimVideoState createState() => _PlanSimVideoState();
}

class _PlanSimVideoState extends State<PlanSimVideo> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _tabController = TabController(length: plansim.length, vsync: this);
    Wakelock.enable();
  }
  ...
  Widget getBody() {
    ...
    return PageView.builder(
        itemCount: plansim.length,
        itemBuilder: (context, index) {
          return VideoPlayerItem(
            videoUrl: plansim[index]['videoUrl'],
            size: size,
            name: plansim[index]['name'],
            caption: plansim[index]['caption'],
            profileImg: plansim[index]['profileImg'],
          );
        });
  }
}
...
List plansim = [
  {
    "videoUrl": "http://13.229.103.9:3000/videos/video_1660246422612.mp4",
    "name": "Samin Rahman",
    "caption": "",
