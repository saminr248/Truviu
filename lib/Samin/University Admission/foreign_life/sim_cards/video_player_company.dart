...
class CompanySimVideoScreen extends StatefulWidget {
  const CompanySimVideoScreen({Key? key}) : super(key: key);

  @override
  State<CompanySimVideoScreen> createState() => _CompanySimVideoScreenState();
}

class _CompanySimVideoScreenState extends State<CompanySimVideoScreen> {
  @override
...
class CompanySimVideo extends StatefulWidget {
  const CompanySimVideo({Key? key}) : super(key: key);

  @override
  _CompanySimVideoState createState() => _CompanySimVideoState();
}

class _CompanySimVideoState extends State<CompanySimVideo> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _tabController = TabController(length: companysim.length, vsync: this);
    Wakelock.enable();
  }
  ...
  Widget getBody() {
    ...
    return PageView.builder(
        itemCount: companysim.length,
        itemBuilder: (context, index) {
          return VideoPlayerItem(
            videoUrl: companysim[index]['videoUrl'],
            size: size,
            name: companysim[index]['name'],
            caption: companysim[index]['caption'],
            profileImg: companysim[index]['profileImg'],
          );
        });
  }
}
...
List companysim = [
  {
    "videoUrl": "http://13.229.103.9:3000/videos/video_1660246304266.mp4",
    "name": "Samin Rahman",
    "caption": "",
