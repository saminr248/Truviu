...
class MustBankVideoScreen extends StatefulWidget {
  const MustBankVideoScreen({Key? key}) : super(key: key);

  @override
  State<MustBankVideoScreen> createState() => _MustBankVideoScreenState();
}

class _MustBankVideoScreenState extends State<MustBankVideoScreen> {
  @override
...
class MustBankVideo extends StatefulWidget {
  const MustBankVideo({Key? key}) : super(key: key);

  @override
  _MustBankVideoState createState() => _MustBankVideoState();
}

class _MustBankVideoState extends State<MustBankVideo> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _tabController = TabController(length: mustbank.length, vsync: this);
    Wakelock.enable();
  }
  ...
  Widget getBody() {
    ...
    return PageView.builder(
        itemCount: mustbank.length,
        itemBuilder: (context, index) {
          return VideoPlayerItem(
            videoUrl: mustbank[index]['videoUrl'],
            size: size,
            name: mustbank[index]['name'],
            caption: mustbank[index]['caption'],
            profileImg: mustbank[index]['profileImg'],
          );
        });
  }
}
...
List mustbank = [
  {
    "videoUrl": "http://13.229.103.9:3000/videos/video_1660247961452.mp4",
    "name": "Samin Rahman",
    "caption": "",
