...
class OpenBankVideoScreen extends StatefulWidget {
  const OpenBankVideoScreen({Key? key}) : super(key: key);

  @override
  State<OpenBankVideoScreen> createState() => _OpenBankVideoScreenState();
}

class _OpenBankVideoScreenState extends State<OpenBankVideoScreen> {
  @override
...
class OpenBankVideo extends StatefulWidget {
  const OpenBankVideo({Key? key}) : super(key: key);

  @override
  _OpenBankVideoState createState() => _OpenBankVideoState();
}

class _OpenBankVideoState extends State<OpenBankVideo> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _tabController = TabController(length: openbank.length, vsync: this);
    Wakelock.enable();
  }
  ...
  Widget getBody() {
    ...
    return PageView.builder(
        itemCount: openbank.length,
        itemBuilder: (context, index) {
          return VideoPlayerItem(
            videoUrl: openbank[index]['videoUrl'],
            size: size,
            name: openbank[index]['name'],
            caption: openbank[index]['caption'],
            profileImg: openbank[index]['profileImg'],
          );
        });
  }
}
...
List openbank = [
  {
    "videoUrl": "http://13.229.103.9:3000/videos/video_1660247820718.mp4",
    "name": "Samin Rahman",
    "caption": "",
