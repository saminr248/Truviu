...
class BestBankVideoScreen extends StatefulWidget {
  const BestBankVideoScreen({Key? key}) : super(key: key);

  @override
  State<BestBankVideoScreen> createState() => _BestBankVideoScreenState();
}

class _BestBankVideoScreenState extends State<BestBankVideoScreen> {
  @override
...
class BestBankVideo extends StatefulWidget {
  const BestBankVideo({Key? key}) : super(key: key);

  @override
  _BestBankVideoState createState() => _BestBankVideoState();
}

class _BestBankVideoState extends State<BestBankVideo> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _tabController = TabController(length: bestbank.length, vsync: this);
    Wakelock.enable();
  }
  ...
  Widget getBody() {
    ...
    return PageView.builder(
        itemCount: bestbank.length,
        itemBuilder: (context, index) {
          return VideoPlayerItem(
            videoUrl: bestbank[index]['videoUrl'],
            size: size,
            name: bestbank[index]['name'],
            caption: bestbank[index]['caption'],
            profileImg: bestbank[index]['profileImg'],
          );
        });
  }
}
...
List bestbank = [
  {
    "videoUrl": "http://13.229.103.9:3000/videos/video_1660254754676.mp4",
    "name": "Samin Rahman",
    "caption": "",
