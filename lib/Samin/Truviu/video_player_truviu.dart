...
class TruviuVideoScreen extends StatefulWidget {
  const TruviuVideoScreen({Key? key}) : super(key: key);

  @override
  State<TruviuVideoScreen> createState() => _TruviuVideoScreenState();
}

class _TruviuVideoScreenState extends State<TruviuVideoScreen> {
  @override
...
class TruviuVideo extends StatefulWidget {
  const TruviuVideo({Key? key}) : super(key: key);

  @override
  _TruviuVideoState createState() => _TruviuVideoState();
}

class _TruviuVideoState extends State<TruviuVideo> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _tabController = TabController(length: truviu.length, vsync: this);
    Wakelock.enable();
  }
  ...
  Widget getBody() {
    ...
    return PageView.builder(
        itemCount: truviu.length,
        itemBuilder: (context, index) {
          return VideoPlayerItem(
            videoUrl: truviu[index]['videoUrl'],
            size: size,
            name: truviu[index]['name'],
            caption: truviu[index]['caption'],
            profileImg: truviu[index]['profileImg'],
          );
        });
  }
}
...
List truviu = [
  {
    "videoUrl": "http://13.229.103.9:3000/videos/video_1660337657196.mp4",
    "name": "Samin Rahman",
    "caption": "",
